...
낯선 이름은 처음 나올 때 무엇인지 드러나게 놓는다. 이름을 쉬운 말로 바꾸는 것이 아니라, 그 이름이 무엇인지 문장이 말하게 하는 것이다. "`checkYn`이 `Y`면 인증을 띄운다"보다 "성인 인증이 필요한지는 `checkYn` 필드가 정한다. 값이 `Y`면 인증 화면을 띄운다"가 읽기 쉽다. 이름은 양쪽 다 살아 있다.
